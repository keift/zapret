...
if [ "${dev}" = true ]; then
  ...
  nfqws_options=$(echo "${blockcheck_results}" | sed -n "/^\* SUMMARY/,/^\$/ { /^\* SUMMARY/d; /^\$/d; p; }" | grep -E "curl_test_https[^ ]* ipv[0-9] ${blockcheck_domain} : nfqws" | grep "ttl" | head -n 1 | sed "s/.*nfqws //" | sed "s|/tmp/zapret|/opt/zapret|g" | sed -z "s/^[[:space:]]*//; s/[[:space:]]*\$//")
fi
