...
EOF
    ...
    while ! dig -p 5353 +tries=1 +time=10 @127.0.0.1 &>/dev/null; do restart_service dnscrypt-proxy; sleep 1; done
...
EOF
    ...
    echo -e "  ${gray}Change the ${green}Custom DNS ${gray}option in the Pi-hole to: ${white}127.0.0.1#5353${reset}"
