...
if [ "${init_system}" = "systemd" ]; then
  curl -fsSL https://raw.github.com/keift/dnsd/refs/heads/main/install.sh | bash -s -- --auto-update &> /dev/null
...
EOF
  ...
  while ! dig -p 53 +tries=1 +time=1 @127.0.0.1 &> /dev/null && ! dig -p 53 +tries=1 +time=1 @::1 &> /dev/null; do
    restart_service dnscrypt-proxy
    restart_service dnscrypt-proxy2
